...
"Tab 1". This reuses gdoc's default-account token, so run it with gdoc's
interpreter:  $(head -1 "$(which gdoc)" | cut -c3-) rename_tab.py ...
"""
import json, os, sys
...
account = json.load(open(f"{cfg}/config.json"))["default_account"]
